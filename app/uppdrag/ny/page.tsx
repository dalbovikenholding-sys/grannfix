...
import { KATEGORIER, type Kategori } from '@/lib/mockData'

const KATEGORILISTA = Object.entries(KATEGORIER) as [Kategori, { ikon: string; etikett: string }][]

const PRISSUGGEST: Record<Kategori, string> = {
        ...
        <div className="mb-10">
          ...
          <div className="flex gap-2 mt-6">
            {[1, 2, 3, 4].map(n => (
              <div
                key={n}
                className={`h-1.5 flex-1 rounded-full transition-colors ${n <= steg ? 'bg-[#1a6b3c]' : 'bg-gray-200'}`}
              />
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                  ...
                  <button
                    ...
                    className={`p-5 rounded-2xl border-2 text-left transition-colors ${
                      form.kategori === k
                        ...
                        : 'border-gray-100 hover:border-[#1a6b3c]/40'
                    }`}
                  >
          ...
          {steg === 2 && (
            <div className="flex flex-col gap-5">
              ...
              <div>
                <label htmlFor="titel" className="block text-sm font-medium text-gray-700 mb-1.5">Titel</label>
                <input
                  id="titel"
                  type="text"
                  value={form.titel}
                  onChange={e => setForm(f => ({ ...f, titel: e.target.value }))}
                  placeholder="T.ex. Flytt av 2:a i centrum"
              ...
              <div>
                <label htmlFor="adress" className="block text-sm font-medium text-gray-700 mb-1.5">Adress</label>
                <input
                  id="adress"
                  type="text"
                  value={form.adress}
              ...
              <div>
                <label htmlFor="datum" className="block text-sm font-medium text-gray-700 mb-1.5">Önskat datum</label>
                <input
                  id="datum"
                  type="date"
                  value={form.datum}
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button
                  type="button"
                  disabled={!form.titel || !form.beskrivning || !form.adress || !form.datum}
                  onClick={handleNext}
          ...
          {steg === 3 && (
            <div className="flex flex-col gap-6">
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button type="button" onClick={handleNext} className="btn-primary flex-1 bg-[#1a6b3c] text-white py-3.5 rounded-full font-semibold">
                  Granska
                </button>
              </div>
            </div>
          ...
          {steg === 4 && (
            <form onSubmit={handlePublish} className="flex flex-col gap-5">
              ...
              <div className="bg-[#f0faf4] rounded-2xl p-6 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{form.adress}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Datum</p>
                    <p className="font-medium text-gray-900">{form.datum}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Pris</p>
                    <p className="font-bold text-[#1a6b3c] text-lg">{form.pris} kr</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="btn-primary flex-1 bg-[#1a6b3c] text-white py-3.5 rounded-full font-semibold">
                  Publicera uppdrag
                </button>
              </div>
            </form>
